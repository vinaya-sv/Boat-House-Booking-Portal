...
function AdminViewOfUser(props){
    ...
    const header=["Booking ID", "Boat ID","Number of people", "Check in", "Check out", "Booked on"];
    // const columns=[        {"Boat ID":12345,"Boat name":"Tide's Tranquility","Check in":"31-01-2024","Check out":"01-02-2024","Status":"Booked"}
    const columns=boatData.map((d)=>({linkto:`/admin-dashboard/all-boats/boat?id=${d.boatId}`,"Booking ID":d.id, "Boat ID":d.boatId,"Number of people":d.numberOfPeople, "Check in":d.checkIn, "Check out":d.checkOut,"Booked on":d.dateOfBooking}))
    const flexRatio=[1,7,2,2,2,2];
    return(
        <div className="admin-user-view-outer">
            <div className="admin-user-view-personal-profile">
                <div className="admin-user-view-personal-profile-picture" style={{backgroundImage:`url('${data.image}')`}}></div>
