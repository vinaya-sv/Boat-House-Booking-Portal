    ...
    return(
        <div className="navbar-outer">
            ...
            <Link to="/contact-us" className="link-tag"><div className="navbar-field">CONTACT US</div></Link>
            {
                isLoggedIn ?
                <>
                    <Link to={(localStorage.getItem("role")=="USER")?"/dashboard/your-profile":(localStorage.getItem("role")=="ADMIN"?"/admin-dashboard/statistics":"")} className="link-tag" ><div className="navbar-field"><CgProfile size={35} className="navbar-profile-icon"/></div></Link>
                </>
                :
